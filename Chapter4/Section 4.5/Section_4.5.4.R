...
pdf(file = paste0(file_name,'.pdf'), width = 10, height = 10)
  ...
  plot(xaxs, log(MOSS_data$Pb), xlab = 'Log(Distance) from Road',
    ylab = 'Log(Pb)', pch = 19, cex = 1.5, cex.lab = 2, cex.axis = 1.5,
    xaxt = 'n', col = 'white', ylim = c(-.25,7.5))
  axis(1, at = c(-10,-5,0,5,10), label = c('10','5','0','5','10'), cex.axis = 1.5)
  ...
  legend(-1.65, 4, legend = c('2001','2006'), pch = c(1,2),
    cex = 2)
  legend(-2.25, 2, legend = c('2001','2006'), lty = c(1,2),
    cex = 2, lwd = 2)
dev.off()
...
file_name = "Moss_Heavy_Pb_OLSvariograms"
...
pdf(file = paste0(file_name,'.pdf'), width = 10, height = 10)
  ...
  plot(vgm_raw$dist, vgm_raw$gamma, cex = 8*vgm_raw$np/max(vgm_raw$np),
  ...
  old_par = par(mar = c(5,5,3,1))
  plot(vgm_resid$dist, vgm_resid$gamma, cex = 8*vgm_resid$np/max(vgm_resid$np),
    pch = 19, xlab = 'Distance (km)', ylab = 'Semivariogram', cex.axis = 1.5, cex.lab = 2)
