...
# remove geometry and use sqrt of response
SO4_data = data.frame(z = sqrt(unlist(st_drop_geometry(SO4obs))), 
	easting = st_coordinates(SO4obs)[,1], 
	northing = st_coordinates(SO4obs)[,2])

# from Section 3.6, remove the outlers
SO4clean = SO4_data[!(1:dim(SO4_data)[1] %in% c(146,153,173)),]
DF = SO4clean
...
# in order to keep spatial distances intact, combine variances first
# so both coordinates are divided by same constant
bothvar = mean(var(DF$easting), var(DF$northing))
xc = (DF$easting - mean(DF$easting))/sqrt(bothvar)
yc = (DF$northing - mean(DF$northing))/sqrt(bothvar)
DF$xc = xc
DF$yc = yc
...
fit_1c <- lm(z ~ poly(xc, yc, degree = 1), data = DF)
fit_2c <- lm(z ~ poly(xc, yc, degree = 2), data = DF)
fit_3c <- lm(z ~ poly(xc, yc, degree = 3), data = DF)
fit_4c <- lm(z ~ poly(xc, yc, degree = 4), data = DF)
fit_5c <- lm(z ~ poly(xc, yc, degree = 5), data = DF)

AOV_1_2 = anova(fit_1c, fit_2c)
AOV_2_3 = anova(fit_2c, fit_3c)
AOV_3_4 = anova(fit_3c, fit_4c)
AOV_4_5 = anova(fit_4c, fit_5c)

tab = data.frame(Surface = c('Planar', 'Quadratic', 'Cubic', 'Quartic',
    'Quintic'), 
  R2 = c(summary(fit_1c)$r.squared, summary(fit_2c)$r.squared, 
    summary(fit_3c)$r.squared, summary(fit_4c)$r.squared, 
    summary(fit_5c)$r.squared),
  s = c(summary(fit_1c)$sigma, summary(fit_2c)$sigma, summary(fit_3c)$sigma,
		summary(fit_4c)$sigma, summary(fit_5c)$sigma),
  ResDF = c(summary(fit_1c)$df[2], summary(fit_2c)$df[2], summary(fit_3c)$df[2],
    summary(fit_4c)$df[2], summary(fit_5c)$df[2]),
