...
estpred_notransform = function(theta, y, sampsize = NULL, X, distmat = NULL,
	autocor_fun, family, Z1 = NULL, Z2 = NULL, mask = NULL,
...
# so just copy that same code from likelihood function
	...
	if(!is.null(Z1)) {
		 revar1 = theta[nparsofar + 1]
		 nparsofar = nparsofar + 1	
	}	 
	if(!is.null(Z2)) {
		 revar2 = theta[nparsofar + 1]
		 nparsofar = nparsofar + 1	
	}	 
