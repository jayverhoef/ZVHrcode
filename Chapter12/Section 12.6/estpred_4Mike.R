...
# sampsize: sample size for binomial data, default is NULL, in which case
#     sample size is assumed to be one for all data
...
# family: distribution conditional on spatial random effects.  Currently choose
#   from 'poisson', 'binomial', 'negbinomial','gamma','invgauss', or 'beta'.
...
# use.nugget: include a nugget effect in the model, default is TRUE.
estpred_4Mike = function(theta, y, sampsize = NULL, X, distmat = NULL,
	autocor_fun, family, Xp = NULL,  dist_op = NULL, dist_pp = NULL, 
	use.nugget = TRUE)
{
...
# so just copy that same code from likelihood function
	...
	if(use.nugget == TRUE) {
		...
		gam_0 = theta[nparsofar] + 1e-6
	}
	if(family %in% c('negbinomial','gamma', 'invgauss', 'beta')) {
		phi = theta[nparsofar + 1]
		nparsofar = nparsofar + 1
	}
  theta_trans = c(gam_0, gam_1, gam_2, phi)
  names(theta_trans) = c('nugget', 'psill', 'range', 'phi')
...
#		wdiffmax = max(abs(wnew - w))
	...
	list(theta_trans = theta_trans, w = w, betahat = betahat, cov_w = mHi, 
		betawtsvarw = betawtsvarw, covbeta = covbeta, covbeta_adj = covbeta_adj,
