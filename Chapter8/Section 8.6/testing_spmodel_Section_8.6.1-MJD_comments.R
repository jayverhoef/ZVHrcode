...
# get the full data
fulldata = as.data.frame(spautor_out4b$data)
...
y = model.response(model.frame(spautor_out4b$formula, fulldata))
...
predcompalt <- sqrt(diag(V[indMiss, indMiss]) -
	...
	2*apply((covb %*% t(Xp)) * (t(Xo) %*% ViVpred), 2, sum) +
	apply(((covb %*% t(ViX)) %*% Vpred) * (t(Xo) %*% ViVpred), 2, sum))
...
# compare
sppreds <- predict(spautor_out4b, se.fit = TRUE, interval = 'prediction')

## fitted

### jay code, spmodel
all.equal(unname(preds[, 1]), unname(sppreds$fit[, 1]))

### se
all.equal(unname(preds[, 2]), unname(sppreds$se.fit))

### lower bound
all.equal(unname(preds[, 3]), unname(sppreds$fit[, 2]))

### upper bound
all.equal(unname(preds[, 4]), unname(sppreds$fit[, 3]))
