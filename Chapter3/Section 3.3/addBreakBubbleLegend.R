...
addBreakBubbleLegend <- function(ybottom, ytop, bubble_labels, 
	cexes, header = NULL, cex.header = NULL, x.header = .2, ...) 
{
  ...
  plot(c(0,1),c(0,1), type ='n', xlab = '', ylab ='',xaxt = 'n', yaxt = 'n',
    bty = 'n')
  for(i in 1:ncexes)
    points(0.1, ybottom + (i-1)/ncexes*(ytop - ybottom), cex = cexes[i])
  for(i in 1:ncexes)
    text(0.2, ybottom + (i-1)/ncexes*(ytop - ybottom), bubble_labels[i], 
    pos = 4, ...)
  text(x.header, ytop, header, cex = cex.header, pos = 4)
  par(old.par)
  return(invisible())	
}
