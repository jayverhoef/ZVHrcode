...
# ANOVA for clear tarp versus no tarp (3rd element in coef(spfitEXP) because
# clear tarp effect is absorbed into the intercept
# using a t-value on a 1-degree-of-freedom test
# do it by hand first
# model matrix for all factors
Xall = model.matrix(z ~ water*tarp, data = caribouDF)
# get modeled covariance matrix from spmodel
# R-tilda (correlation matrix, without overall variance
Sigma = covmatrix(spfitEXP)
# variance-covariance matrix of the estimated fixed effects
XSigiXinv = solve(t(Xall) %*% solve(Sigma) %*% Xall)
# create the constrast
ell = c(0, 0, 1, 0, 0, 0)
# compute the t-value
tval = (t(ell) %*% coef(spfitEXP))/
	sqrt(t(ell) %*% XSigiXinv %*% ell)
tval
# compute the probability
2*(1 - pt(tval, df = 24))
# or use the Chi-squared formula
Cval = (t(ell) %*% coef(spfitEXP))^2/
	(t(ell) %*% XSigiXinv %*% ell)
Cval
...
# Compare the shade tarp to the average of the clear tarp and control
ell = c(0, 0, -0.5, 1, 0, 0)
# compute the t-value
tval = (t(ell) %*% coef(spfitEXP))/
	sqrt(t(ell) %*% XSigiXinv %*% ell)
...
# Estimate the nitrogen content under shade conditions and low moisture 
ell = c(1, 0, 0, 1, 0, 0)
# Use BLUPTE
...
# test as Chi-squared
1-pchisq((coef(spfitEXP)[2]/sqrt(vcov(spfitEXP)[2,2]))^2, df = 1)
# test as standard normal
2*(1-pnorm(coef(spfitEXP)[2]/sqrt(vcov(spfitEXP)[2,2])))
